...
from domain.shopify_collection import schema, crud
from domain.booksen_category import crud as booksen_category_crud

router = APIRouter(
    prefix="/api/shopify/collection",
)
...
def create_item(itfcd_id: int, create: schema.Create, db: Session = Depends(get_db)):
    booksen_category = booksen_category_crud.get_item(db, id=itfcd_id)
    if not booksen_category:
        raise HTTPException(status_code=404, detail="없는 booksen_category_id 입니다.")
    crud.create_item(db, booksen_category=booksen_category, create=create)
