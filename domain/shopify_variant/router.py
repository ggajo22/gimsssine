...
from domain.shopify_variant import schema, crud
from domain.shopify_product import crud as shopify_product_crud

router = APIRouter(
    prefix="/api/shopify/variant",
)
...
def create_item(shopify_product_id: int, create: schema.Create, db: Session = Depends(get_db)):
    ...
    crud.create_item(db, shopify_product=shopify_product, create=create)
